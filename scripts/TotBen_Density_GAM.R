...
#convert all benthic (school and individual) count to density
...
# geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
BvS
correlation_coefficient <- cor(site_DE$NASC_10_1m, site_DE$TBen)
...
TF<- site_DE%>% #TF = TotalFish (no herring)
  dplyr::select(c(Site_ID,TBen,Average_5m_slope,Std_Dev_Slope, Ratio, Cumulative_LG_DZ_Area, 
                  NASC_10_1m, Depth_mean_5_MAN))
str(TF)

###################check outliers ###########

explanatory_vars <- names(TF)[!names(TF) %in% "TBen"]
explanatory_vars
plots_list <- list()
...
for (var in explanatory_vars) {
  plot_title <- paste("TBen  vs", var)
  plot <- ggplot(TF, aes(x = !!sym(var), y = TBen)) +
    geom_point() +
    geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) +
...
model <- lm(TBen ~ ., data = site_explan)
vif(model)
...
model2 <- lm(TBen ~ ., data = site_explan2)
vif(model2)

#all under 3 so okay now
...
#NASC
lmNASC <- lm(TBen ~ NASC_10_1m, data = TF) #total fish is offset by log transformed sampling volume
lmNASC
summary(lmNASC)
...
#stdev_slope
lmSTDslope <- lm(TBen ~ Std_Dev_Slope , data = TF) #total fish is offset by log transformed sampling volume
lmSTDslope
summary(lmSTDslope)
...
#Ratio_s
lmRatio <- lm(TBen ~ Ratio , data = TF) #total fish is offset by log transformed sampling volume
lmRatio
summary(lmRatio)
...
lmDZ <- lm(TBen ~ Cumulative_LG_DZ_Area  , data = TF) #total fish is offset by log transformed sampling volume
lmDZ
summary(lmDZ)
...
#Depth_mean_5_MAN_s
lmD <- lm(TBen ~ Depth_mean_5_MAN  , data = TF) #total fish is offset by log transformed sampling volume
lmD
summary(lmD)
...
#original full model
# M1 <- gam(TBen ~ s(Average_5m_slope) + s(Std_Dev_Slope) +
#             s(Cumulative_LG_DZ_Area) + s(NASC_10_1m) + s(Depth_mean_5_MAN), #remove
#           data = TF)

M1 <- gam(TBen ~ + s(Std_Dev_Slope, k=4)  + s(NASC_10_1m, k=4) , #remove
          data = TF)

M1 <- gam(TBen ~ + Std_Dev_Slope  + NASC_10_1m , #remove
          data = TF)
#model basically ends up being linear (but seems like there are issues with residuals 
# interesting that STD slope is significant in this model and not benthic model - are STD slope and deadzone
#colinear so bascially explaining the same effect?)
...
plot(TF$NASC_10_1m, TF$TBen, cex = 1.1, pch = 16, main = "Gaussian GAM", 
...
smoothed_line4 <- smooth.spline(TF$Std_Dev_Slope, residuals, spar = 1)
...
plot(TF$Std_Dev_Slope, residuals, xlab = "Deadzone", ylab = "Pearson Residuals", 
