...
#make sub type factor
site_DE$TopSub<-as.factor(site_DE$TopSub)
...
str(site_DE)
BvS<-ggplot(site_DE, aes(x=NASC_10_1m, y=Ben))+
  geom_point()+
  geom_smooth(method="lm", se=FALSE)+
  geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
# geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
BvS
correlation_coefficient <- cor(site_DE$Average_5m_slope, site_DE$Ben)
correlation_coefficient

#create coplot to look at interactions

coplot(Ben ~ NASC_10_1m|Average_5m_slope, data = site_DE,
       panel = function(x, y, ...) {
         points(x, y, ...)
...
BvS<-ggplot(site_DE, aes(x=Average_5m_slope , y=Ben))+
  geom_point()+
  geom_smooth(method="lm", se=FALSE)+
  geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
# geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
...
BvS<-ggplot(site_DE, aes(x=Std_Dev_Slope , y= Ben))+
  geom_point()+
  geom_smooth(method="lm", se=FALSE)+
  geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
# geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
...
BvS<-ggplot(site_DE, aes(x=Cumulative_LG_DZ_Area, y=Ben))+
  geom_point()+
  geom_smooth(method="lm", se=FALSE)+
  geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
# geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
...
BvS<-ggplot(site_DE, aes(x=Average_Depth, y=Ben))+
  geom_point()+
  geom_smooth(method="lm", se=FALSE)+
  geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
# geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
...
ggplot(site_DE, aes(x = NASC_10_1m, y = Ben, color = Average_5m_slope)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +  # You can adjust colors as per your preference
  labs(x = "NASC_10_1m", y = "Ben", color = "Average_5m_slope")

ggplot(site_DE, aes(x = NASC_10_1m, y = Ben, color = Std_Dev_Slope)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +  # You can adjust colors as per your preference
  labs(x = "NASC_10_1m", y = "Ben", color = "Std_Dev_Slope")

ggplot(site_DE, aes(x = NASC_10_1m, y = Ben, color = Cumulative_LG_DZ_Area)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +  # You can adjust colors as per your preference
  labs(x = "NASC_10_1m", y = "Ben", color = "Cumulative_LG_DZ_Area")


#As DZ area increases benthic abundance decreases (cor = -.12)
#As NASC increased DZ area increases (cor = +.46)


#######First run model with density of benthic fish as response################

TF<- site_DE%>% #TF = TotalFish (no herring)
  dplyr::select(c(Site_ID,Ben,Average_5m_slope,Std_Dev_Slope, Ratio, Cumulative_LG_DZ_Area, 
                  NASC_10_1m, Average_Depth, TopSub, TopSlope))
str(TF)

###################check outliers ###########

explanatory_vars <- names(TF)[!names(TF) %in% "Ben"]
explanatory_vars
plots_list <- list()
...
for (var in explanatory_vars) {
  plot_title <- paste("Ben  vs", var)
  plot <- ggplot(TF, aes(x = !!sym(var), y = Ben)) +
    geom_point() +
    geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) +
...
#NASC
lmNASC <- lm(Ben ~ NASC_10_1m, data = TF) #total fish is offset by log transformed sampling volume
lmNASC
summary(lmNASC)
...
#stdev_slope
lmSTDslope <- lm(Ben ~ Std_Dev_Slope , data = TF) #total fish is offset by log transformed sampling volume
lmSTDslope
summary(lmSTDslope)
...
#Ratio_s
lmRatio <- lm(Ben ~ Ratio , data = TF) #total fish is offset by log transformed sampling volume
lmRatio
summary(lmRatio)
...
lmDZ <- lm(Ben ~ Cumulative_LG_DZ_Area  , data = TF) #total fish is offset by log transformed sampling volume
lmDZ
summary(lmDZ)
...
#Depth_mean_5_MAN_s
lmD <- lm(Ben ~ Depth_mean_5_MAN  , data = TF) #total fish is offset by log transformed sampling volume
lmD
summary(lmD)
...
Avg_20m_Slope<-bin_complete%>%
  dplyr::select(Site_ID, BinID, )

BvS<-ggplot(TF, aes(x=Average_5m_slope, y=Ben))+
  geom_point()+
  geom_smooth(method="lm", se=FALSE)+
  geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
# geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
...
BvS<-ggplot(TFstd, aes(x=Std_Dev_Slope_s, y=Ben))+
  geom_point()+
  geom_smooth(method="lm", se=FALSE)+
  geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
# geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
...
BvS<-ggplot(TFstd, aes(x=Cumulative_LG_DZ_Area_s, y=Ben))+
  geom_point()+
  geom_smooth(method="lm", se=FALSE)+
  geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
# geom_text(aes(label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3) 
...
correlation_coefficient <- cor(site_DE$NASC_5_1m, site_DE$Ben)
correlation_coefficient

#####glm models####

#full model 
M1 <- glm(Ben ~ Average_5m_slope_s + Std_Dev_Slope_s +
            Cumulative_LG_DZ_Area_s + NASC_10_1m_s + Average_Depth_s, #remove
          family = gaussian,  data = TFstd)
summary(M1)
...
#try glm with deadzone removed
M2 <- glm(Ben ~ Average_5m_slope + Std_Dev_Slope +
            NASC_10_1m + Depth_mean_5_MAN, #remove
          family = gaussian,  data = TF)
summary(M2)
...
##try with deadzone added back in as interaction to account for negative corrlation found in residuals

# M3 <- glm(Ben ~ Average_5m_slope + Std_Dev_Slope +
#             Depth_mean_5_MAN + NASC_10_1m +
#             Cumulative_LG_DZ_Area:NASC_10_1m - Cumulative_LG_DZ_Area, #remove
#           family = Gamma(link="inverse"),  data = TF)
# summary(M3)

# M3 <- glm(Ben ~ Average_5m_slope + Std_Dev_Slope +
#             Average_Depth + NASC_10_1m +
#             Cumulative_LG_DZ_Area:NASC_10_1m - Cumulative_LG_DZ_Area, #remove
#           family = gaussian,  data = TF)
# summary(M3)
# 
# M3 <- glm(Ben ~ NASC_10_1m:Average_5m_slope + NASC_10_1m:Std_Dev_Slope +
#             Average_Depth + NASC_10_1m +
#             Cumulative_LG_DZ_Area:NASC_10_1m - Cumulative_LG_DZ_Area, #remove
#           family = gaussian,  data = TF)
# summary(M3)

### BEST MODEL#### - there appears to be a negative interaction between slope and fish density####
#the higher the slope the lower the density.  But the higher the NASC the higher the fish density at
#low slope sites
...
# #trying quadratic to account for increase then decrease on benthic density (not sure if this is correct)
# M3 <- glm(Ben ~ NASC_10_1m:I(Average_5m_slope^2)+ 
#                       NASC_10_1m 
#                     , #remove
#                     family = gaussian,  data = TF)
# summary(M3)
...
#seeing some heteroscedasticity (is this a problem or just due to small sample size?) 
r <- simulateResiduals(M3, n = 1000, plot = TRUE)
...
####gam model####
Mgam<-gam(Ben ~  NASC_10_1m +
            s(NASC_10_1m, by= Average_5m_slope, k=5),
          data=TF)
summary(Mgam)
k.check(Mgam)
plot(Mgam)
par(mfrow = c(2, 2))
gam.check(Mgam)  # a bit of a positive linear pattern in resp v fitted

M3 <- gam(Ben ~ s(Average_5m_slope, k=4) + s(Std_Dev_Slope, k=4) +
            Average_Depth + NASC_10_1m + s(Cumulative_LG_DZ_Area, k=4)+
            s(NASC_10_1m, by= Average_5m_slope, k=5), #remove
           data = TF)
summary(M3)

##try removing non-sign predictors

M4 <- glm(Ben ~ Average_5m_slope + Std_Dev_Slope +
            Depth_mean_5_MAN + NASC_10_1m + Cumulative_LG_DZ_Area
             , #remove
          family = gaussian,  data = TF)
summary(M4)
...
#pattern in rugosity resid plot when removed from model
#pattern in depth resid plot when removed from model
...
ggplot(TF2) +
  geom_point(aes(x = Average_5m_slope,
                 y = resid)) +
  geom_text(aes(x = Average_5m_slope,
                y = resid,
                label = Site_ID), nudge_x = 0.1, nudge_y = 0.1, size = 3)+
  geom_smooth(aes(x = Average_5m_slope,
                  y = resid),
              method = "lm") 
...
library(DHARMa)
r <- simulateResiduals(M1, n = 1000, plot = TRUE)
r <- simulateResiduals(M2, n = 1000, plot = TRUE)  #problems in resid vs. predicted plot from not including deadzone
r <- simulateResiduals(M3, n = 1000, plot = TRUE)
r <- simulateResiduals(M4, n = 1000, plot = TRUE)

#check dispersion
testDispersion(M1)

M4 <- glm(Ben ~ NASC_10_1m +
            Cumulative_LG_DZ_Area:NASC_10_1m - Cumulative_LG_DZ_Area, #remove
          family = gaussian,  data = TF)
summary(M4)

########################### Her nerdiness ends here.
  

lmSTDslope <- lm( ~ residuals , data = TF) #total fish is offset by log transformed sampling volume
lmSTDslope
summary(lmSTDslope)
...
#####previous models####
M1 <- gam(Ben ~  s(Cumulative_LG_DZ_Area) + s(NASC_10_1m), #remove
           data = TF, gamma=1.4)

#can also model isotropically (compare plots)
# M1 <- gam(Ben ~  s(Cumulative_LG_DZ_Area,NASC_10_1m), #remove
#           data = TF, gamma=1.4)
